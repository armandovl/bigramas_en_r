...
#https://www.youtube.com/watch?v=522cn9imFfc&t=1004s
##################################################
...
#texto01 <- pdftools::pdf_text("buenos_capsulas.pdf")
#texto02 <- pdftools::pdf_text("buenos_soluble.pdf")

texto01 <- readLines("buenos_capsulas.txt")
texto02 <- readLines("buenos_soluble.txt")
...
# Juntamos todas las paginas del libros
...
# Quitamos los espacios de encabezado y pie de pagina
...
# Vamos a hacer un poco de limpieza de texto manual
# ------------------------------------------
...
lexiconSW <- append(lexiconSW,c("capítulo","d"," ")) #agregar nuevas stopwords
...
#graficamos las palabras mas recurrentes
word_counts %>%
  head(40) %>% #arriba de 40 menciones
...
listadoBigramas<-title_word_pairs[which(title_word_pairs$n>10),] #10 interacciones
set.seed(1234)

#graficar red
title_word_pairs %>%
  filter(n >= 20) %>% #filtro para grafico mas de 20 interacciones
